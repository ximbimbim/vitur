	...
	<div data-ng-controller="searchCtrl">
		...
		<div class="row hide" id="searchApp_viewer">			
			...
			<div class="col-md-8">
				...
				<div data-ng-show="pageData().length > 0">
					...
					<div class="vtr-tour-item" data-ng-repeat="hotel in pageData()">
						<div class="vtr-tour-img"><img ng-src="<?= Url::to( '@web/img/hotel' ) ?>_{{hotel.id}}.jpg" class="img-responsive" /></div>
						<div class="vtr-tour-body">
							<div style="display: inline-block; width: 100%; height: 100%; position: relative">
								...
								<div class="vtr-features-box">									
									<span data-ng-repeat="feature in hotel.features" class="fa fa-{{feature}}"></span>
								</div>
								<div class="vtr-price-box">
									<h4 class="vtr-price-value"><sub>R$</sub> {{hotel.price.full}}<sup>,{{hotel.price.cent}}</sup></h4>
									<h3 class="vtr-price-parcel"><sub>{{hotel.parcel.quant}}x</sub> R$ {{hotel.parcel.full}}<sup>,{{hotel.parcel.cent}}</sup></h3>
								</div>
							</div>
						</div>
					</div> <!-- ./VTR-TOUR-ITEM -->
					...
					<div class="vtr-tool-bar">
						<div class="vtr-pagination" page="{{page}}" pageCount="{{pageCount}}"></div>
					</div> <!-- ./VTR-ORDER-BAR -->
				</div> <!-- ./#RESULT_DATA -->
			</div> <!-- ./COL-LG -->
		</div> <!-- ./ROW -->
